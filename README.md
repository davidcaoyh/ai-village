...
bash scripts/dev.sh test                 # 33 tests, offline, about a second
...
village/     the agent system; runs headless, never imports server/
  ...
  orchestrator.py round-robin scheduling and the four stop conditions
...
tests/       33 tests, no network, no API key
```
