...
| `tests/` | 33 offline tests | A test suite that needs an API key is one you stop running. The loop is driven through an injected `chat_fn`, which is the same seam `--fake` uses. |
...
**Role here:** it is the entire site - seven route functions in ~140 lines.
...
| Tool | Role | Why |
|---|---|---|
| **pytest** | test runner | Plain functions and `assert`; fixtures give each test its own temp database. 33 tests, no network, ~1s. |
